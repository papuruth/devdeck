...
## 1.2 JWT Toolkit — EXPAND existing (`/jwt-decoder`) ✅ COMPLETE
...
- ✅ **Algorithm selector:** Full 5-family support — HMAC (HS256/384/512), RSA (RS256/384/512), RSA-PSS (PS256/384/512), ECDSA (ES256/384/512), EdDSA (Ed25519). Grouped `<select>` with optgroups.
- ✅ **Header CodeArea:** Pre-filled with `{"alg": "HS256", "typ": "JWT"}` — editable. `alg` field syncs automatically when algorithm changes.
- ✅ **Payload CodeArea:** Pre-filled with template: `{"sub": "user_id", "iat": <now>, "exp": <now+3600>}`
- ✅ **Secret input (HMAC):** Text input with Base64 toggle, disabled + tooltip when header is invalid. Restores default when cleared and algo changes.
- ✅ **Key pair input (asymmetric):** Private key PEM textarea + "Generate Key Pair" button (Web Crypto API). Public key display with copy button. Both disabled + tooltip when header is invalid. Key pair resets on algorithm change.
- ✅ **Live generation:** Debounced 300ms — token regenerates as user types
- ✅ **Output:** Full token string in `CodeArea` (readonly), `BadgeRow` showing alg + expiry
- ✅ **"Send to Decoder" button:** Uses `useToolChain` → chains generated token to Decode tab
- ✅ **Library:** `jose` v6.2.3 — `SignJWT`, `importPKCS8`, `importSPKI`, `jwtVerify`
- ✅ **Error handling:** Invalid JSON in header/payload → inline ErrorBadge; missing secret/key → warning
...
3. ✅ **JWT Toolkit expansion** — complete (all 5 algorithm families, key pair generation, full UX)
