...
function getContrastLabel(ratio: number): ContrastLabel {
...
const ContrastSwatch = styled.div<{ $color?: string; $border?: string; $textColor?: string }>`
    width: 40px;
    height: 40px;
    border-radius: 4px;
    background: ${(p) => p.$color};
...
const WcagBadge = styled.span<{ $type?: string }>`
    font-size: 12px;
    font-weight: 600;
    font-family: "Inter", sans-serif;
    padding: 2px 8px;
...
export default function ColorConverter() {
    ...
    const [rgb, setRgb] = useState<RGBColor>({ r: 34, g: 204, b: 153 });
    const [hsl, setHsl] = useState<HSLColor>({ h: 160, s: 71, l: 47 });
    ...
    const updateFromRgb = useCallback(
        (field: keyof RGBColor, raw: string) => {
    ...
    const updateFromHsl = useCallback(
        (field: keyof HSLColor, raw: string) => {
                    ...
                    <InputRow>
                        {["r", "g", "b"].map((ch) => (
                            <React.Fragment key={ch}>
                                ...
                                <ChannelInput type="number" min={0} max={255} value={rgb[ch as keyof RGBColor]} onChange={(e) => updateFromRgb(ch as keyof RGBColor, e.target.value)} />
                            </React.Fragment>
                        ))}
                    </InputRow>
                </InputSection>
                ...
                <InputSection>
                    ...
                    <InputRow>
                        {[
                            ...
                            { k: "s" as const, label: "S" },
                            { k: "l" as const, label: "L" }
                        ].map(({ k, label }) => (
