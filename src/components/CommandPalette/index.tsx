...
function usePlaceholder(active: boolean): { text: string; fading: boolean } {
...
export default function CommandPalette({ open, onClose }: CommandPaletteProps) {
    ...
    const { sections, selectables } = useMemo(() => {
        const secs: Array<{ id: string; label: string; items: Array<PaletteEntry & { _score?: number; _positions?: number[] }> }> = [];
        ...
        if (isBlogMode) {
            ...
            const matched = blogSubQuery
                ? fuzzyFilterWithPositions(
                      BLOG_GUIDES,
                      blogSubQuery,
                      (g: PaletteEntry) => [g.label, g.description || "", ...(g.keywords || [])]
                  ).map(({ item, score, positions }: any) => ({ ...item, _score: score, _positions: positions }))
                : BLOG_GUIDES;
            ...
            const filtered = categorySlug ? ENRICHED_TOOLS.filter((t: PaletteEntry) => t.category.startsWith(categorySlug)) : ENRICHED_TOOLS;
            ...
            const matched = cmdQuery
                ? fuzzyFilterWithPositions(commands, cmdQuery, (c: PaletteEntry) => [c.label, c.description || "", ...(c.keywords || [])]).map(
                      ({ item, score, positions }: any) => ({ ...item, _score: score, _positions: positions })
                  )
                : commands;
            if (matched.length > 0) {
            ...
            const related = getRelatedToolEntries(pathname ?? "").filter((t: PaletteEntry) => !shownRoutes.has(t.route));
            if (related.length > 0) {
                related.forEach((t: PaletteEntry) => shownRoutes.add(t.route));
                secs.push({ id: "related", label: "Related", items: related });
            }

            // Fill remaining slots with suggested tools, cap total at 8
            const suggested = SUGGESTED_TOOLS.filter((t: PaletteEntry) => !shownRoutes.has(t.route));
            ...
            const matched = fuzzyFilterWithPositions(allItems, query, (item: PaletteEntry) => [item.label, item.description || "", ...(item.keywords || [])]).map(
                ({ item, score, positions }: any) => ({ ...item, _score: score, _positions: positions })
            );

            const matchedActions = matched.filter((i: any) => i.kind === "action");
            const matchedGuides = matched.filter((i: any) => i.kind === "guide");
            const matchedTools = matched.filter((i: any) => i.kind !== "action" && i.kind !== "guide");
        ...
        const smartItem = smartDetection ? [{ kind: "smart" as const, detection: smartDetection }] : [];
        const all = [...smartItem, ...secs.flatMap((s) => s.items)];
        return { sections: secs, selectables: all };
    }, [query, isBlogMode, blogSubQuery, isCategoryMode, isCommandMode, categorySlug, actions, commands, smartDetection, pathname]);
    ...
    type SmartItem = { kind: "smart"; detection: { route: string; label: string } };
    type SelectableItem = (PaletteEntry & { _score?: number; _positions?: number[] }) | SmartItem;
    ...
    const highlightLabel = (label: string, positions?: number[]): React.ReactNode => {
        if (!positions || positions.length === 0) return label;
        const posSet = new Set(positions);
        const segments: Array<{ highlighted: boolean; text: string; key: string }> = [];
        let i = 0;
    ...
    const renderedSections = sections.map((sec) => {
        const startIndex = sectionItemIndex;
                        ...
                        <div key={sec.id}>
                            ...
                            {sec.items.map((item, i) => {
                                ...
                                const iconEl = item.icon
                                    ...
                                    : CATEGORY_EMOJI[item.category as keyof typeof CATEGORY_EMOJI] || "🛠️";
                                ...
                                return (
                                    ...
                                    >
                                        ...
                                        <ItemContent>
                                            <ItemLabel $dimmed={(item._score ?? 0) > 0 && (item._score ?? 0) < 15}>
