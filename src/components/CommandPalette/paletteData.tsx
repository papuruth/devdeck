...
import { ContentCopy, DeleteOutlined, Fingerprint, Home } from "@mui/icons-material";
...
export interface PaletteEntry {
    ...
    keywords?: string[];
    kind: "tool" | "guide" | "action" | "command" | "recent";
    run?: () => void;
}
...
export const ENRICHED_TOOLS: PaletteEntry[] = GLOBAL_CONSTANTS.OPERATIONS_ITEMS.map((tool) => ({
    ...tool,
    kind: "tool" as const,
...
export function buildActions({ onClose, navigateTo }: { onClose: () => void; navigateTo: (route: string) => void }): PaletteEntry[] {
    return [
        {
            id: "generate-uuid",
            label: "Generate UUID",
...
export const SUGGESTED_TOOLS: PaletteEntry[] = SUGGESTED_TOOL_ROUTES.map((route) => ENRICHED_TOOLS.find((t) => t.route === route)).filter(
    (t): t is PaletteEntry => Boolean(t)
);

// ── Recent tools (frequency × recency_decay ranked) ──────────────────────────
type RecentEntry = PaletteEntry & { _rankScore: number };
export function getRecentToolEntries(): RecentEntry[] {
    const freq = storage.getToolFrequency();
    return storage
        .getRecentTools()
        .map((route: string, idx: number): RecentEntry | null => {
            const tool = ENRICHED_TOOLS.find((t) => t.route === route);
            if (!tool) return null;
            const frequency = (freq as Record<string, number>)[route] || 1;
            const recencyDecay = 1 / (idx + 1);
            return { ...tool, kind: "recent" as const, _rankScore: frequency * recencyDecay };
        })
        .filter((x): x is RecentEntry => Boolean(x))
...
export function getRelatedToolEntries(currentRoute: string): PaletteEntry[] {
    const routes = (RELATED_TOOLS as Record<string, string[]>)[currentRoute] || [];
    return routes.map((route: string) => ENRICHED_TOOLS.find((t) => t.route === route)).filter((t): t is PaletteEntry => Boolean(t));
}

// ── Command mode actions (only shown when user types ">") ─────────────────────
export function buildCommands({ onClose }: { onClose: () => void }): PaletteEntry[] {
    return [
        {
            ...
            category: "command",
            icon: <DeleteOutlined sx={{ fontSize: "1.1rem" }} />,
