...
const ALGOS = [
    { id: "md5" as const, label: "MD5", fn: (s: string) => CryptoJS.MD5(s).toString() },
    { id: "sha1" as const, label: "SHA-1", fn: (s: string) => CryptoJS.SHA1(s).toString() },
    { id: "sha256" as const, label: "SHA-256", fn: (s: string) => CryptoJS.SHA256(s).toString() },
    { id: "sha512" as const, label: "SHA-512", fn: (s: string) => CryptoJS.SHA512(s).toString() }
];

function hashWordArray(id: string, wa: CryptoJS.lib.WordArray): string {
    switch (id) {
        case "md5":
...
const DropTarget = styled.div<{ $active?: boolean }>`
...
export default function HashGenerator() {
    ...
    const onDrop = useCallback((accepted: File[]): void => {
        ...
        reader.onload = (e: ProgressEvent<FileReader>): void => {
            const wa = CryptoJS.lib.WordArray.create((e.target as FileReader)?.result as ArrayBuffer);
            const computed: Record<string, string> = {};
            ALGOS.forEach(({ id }) => {
        ...
        <ToolLayout>
            ...
            <Panel>
                ...
                {hasOutput ? (
                    ...
                    <EmptyState>
                        <span style={{ fontSize: 26, fontFamily: "var(--font-mono)" }}>#</span>
