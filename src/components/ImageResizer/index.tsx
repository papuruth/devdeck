...
function centerAspectCrop(mediaWidth: number, mediaHeight: number, aspect: number | undefined): PercentCrop {
    return centerCrop(makeAspectCrop({ unit: "%", width: 90 }, aspect ?? 1, mediaWidth, mediaHeight), mediaWidth, mediaHeight);
}
...
export default function ImageResizer() {
    ...
    const [cropDims, setCropDims] = useState<CropDims | null>(null);
    ...
    const prevCropRef = useRef<PixelCrop | null>(null);
    const outWRef = useRef("");
    const outHRef = useRef("");

    const handleSelectedFiles = useCallback((acceptedFiles: File[]) => {
        const loaderId = String(Date.now());
        try {
    ...
    const handleAspectSelect = (value: number | string | undefined) => {
        const preset = ASPECT_PRESETS.find((p) => p.value === value);
        setSelectedPresetLabel(preset?.label ?? "Free");
        const ratio = value === ORIGINAL_ASPECT ? originalAspect ?? undefined : (value as number | undefined);
        setAspect(ratio);
        if (ratio !== undefined && ratio !== null && imgRef.current) {
            const { width, height } = imgRef.current;
            userEditedDimsRef.current = false;
    ...
    useDebounceEffect(
        async () => {
            if (completedCrop?.width && completedCrop?.height && imgRef.current && previewCanvasRef.current) {
                ...
                await canvasPreview(imgRef.current, previewCanvasRef.current, completedCrop, scale, rotate, tw, th);
                updateActualSize();
            }
        },
        100,
                    ...
                    <ControlRow>
                        ...
                        <SliderWrap>
                            <input
                                ...
                                step={0.1}
                                value={scale}
                                style={{ "--fill": scaleFill } as React.CSSProperties}
                                disabled={!imgSrc}
                    ...
                    <ControlRow>
                        ...
                        <SliderWrap>
                            <input
                                ...
                                step={1}
                                value={rotate}
                                style={{ "--fill": rotateFill } as React.CSSProperties}
                                disabled={!imgSrc}
                                onChange={(e) => {
                    ...
                    {imgSrc ? (
                        <CropContainer>
                            <ReactCrop
                                ...
                                aspect={aspect}
                            >
                                {/* react-image-crop requires a raw <img> ref to track crop geometry */}
