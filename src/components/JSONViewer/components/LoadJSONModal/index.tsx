...
function getURLBorderColor(state: string | undefined) {
    if (state === "valid") return "#22cc99";
    if (state === "invalid") return "#ef4444";
    return "var(--border-color)";
}

function getURLState(val: string): "empty" | "valid" | "invalid" {
    if (!val) return "empty";
    try {
        const u = new URL(val);
...
const URLInput = styled.input<{ $state?: string }>`
    width: 100%;
...
const ValidationMark = styled.span<{ $valid?: boolean }>`
    font-size: 12px;
    font-weight: 600;
...
export default function LoadJSONModal({ open, onClose, onLoad }: LoadJSONModalProps) {
    ...
    const handleURLLoad = useCallback(async () => {
        ...
        try {
            ...
            toast.error((error as any).message || "Failed to load JSON");
        }
        setLoading(false);
