...
export default function NumberBaseConverter() {
    ...
    const { results, error } = useMemo(() => {
        ...
        if (Number.isNaN(decimal))
            return { results: null, error: L.invalidInputError.replace("[INPUT]", input.trim()).replace("[BASE]", String(sourceBase)) };
        return {
            results: OUTPUTS.map(({ base, label, prefix }: { base: number; label: string; prefix: string }) => ({
                label,
        ...
        <ToolLayout>
            ...
            <Panel>
                ...
                {results ? (
                    ...
                    <EmptyState>
                        <span style={{ fontSize: 22, fontFamily: "var(--font-mono)" }}>0x</span>
