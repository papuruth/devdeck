...
function getPasswordStrengthLabel(score: number | undefined) {
    if (score === undefined || score <= 2) return L.weakLabel;
        ...
        <ToolLayout>
            <Panel>
                ...
                {CHAR_TYPES.map(({ key, label }) => (
                    <RuleRow key={key}>
                        <StyledCheckbox name={key} checked={!compositionRule[key as keyof typeof compositionRule].forbidden} onChange={handleCompositionChange} />
                        <RuleLabel>{label}</RuleLabel>
                        {!compositionRule[key as keyof typeof compositionRule].forbidden && (
                            <>
                                <MinLabel>{L.minLabel}</MinLabel>
                                <MinSelect name={key} value={compositionRule[key as keyof typeof compositionRule].min} onChange={handleCompositionLength}>
