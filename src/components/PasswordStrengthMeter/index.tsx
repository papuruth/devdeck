...
function getStrengthLabel(score: number | undefined): string {
    if (score === undefined || score <= 1) return L.weak;
    if (score === 2) return "Fair";
    if (score === 3) return L.strong;
    return L.veryStrong;
}

function getStrengthColor(score: number | undefined): string {
    if (score === undefined || score <= 1) return "#ef4444";
...
const CharPill = styled.span<{ $active?: boolean }>`
    font-size: 13px;
    font-family: "Inter", sans-serif;
    font-weight: 500;
    padding: 6px 12px;
...
export default function PasswordStrengthMeter() {
    ...
    const analyze = useCallback(() => {
        ...
        const rawSecs = result.crack_times_seconds?.online_no_throttling_10_per_second ?? 0;
        const secs = typeof rawSecs === "number" ? rawSecs : Number(rawSecs) || 0;
        setAnalysis({
            score: result.score,
            ...
            <Panel>
                ...
                <PasswordInputWrap>
                    <PasswordInput
                        ...
                        spellCheck={false}
                    />
                    <ShowToggle onClick={(): void => setShowPassword((v) => !v)} title={showPassword ? L.hideLabel : L.showLabel}>
