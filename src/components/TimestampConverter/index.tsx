...
function formatInTz(unixSec: number, tz: string): string {
    try {
...
const CalendarTrigger = styled.button<{ $empty?: boolean }>`
    width: 100%;
    background: var(--bg-input);
...
const TzOption = styled.div<{ $active?: boolean }>`
    display: flex;
    align-items: center;
...
const TzOptionOffset = styled.span<{ $active?: boolean }>`
    font-size: 10px;
    font-family: var(--font-mono);
...
function TzPicker({ value, onChange }: TzPickerProps) {
    ...
    useEffect(() => {
        function handleOutside(e: MouseEvent): void {
            if (triggerRef.current && !triggerRef.current.contains(e.target as Node) && (!dropRef.current || !dropRef.current.contains(e.target as Node))) {
                setOpen(false);
                setSearch("");
            }
        }
    ...
    const filtered = useMemo(
        () =>
            ALL_TIMEZONES.filter(
                (tz: string) => tz.toLowerCase().includes(search.toLowerCase()) || TZ_OFFSETS[tz].toLowerCase().includes(search.toLowerCase())
            ),
        [search]
    );
...
function DateTimePicker({ onChange }: DateTimePickerProps) {
    ...
    const emit = useCallback(
        (day: Date | undefined, time: string): void => {
            if (!day) return;
            const [h, m] = time.split(":");
            const dt = new Date(day);
            dt.setHours(parseInt(h ?? "0", 10), parseInt(m ?? "0", 10), 0);
            ...
            {open &&
                ReactDOM.createPortal(
                    <CalendarDropdown ref={dropRef} style={{ top: dropPos.top, left: dropPos.left }}>
                        <DayPicker mode="single" selected={selected} onSelect={handleSelect} defaultMonth={selected} />
                        <CalendarTimeRow>
...
export default function TimestampConverter() {
    ...
    const handleCopy = useCallback((label: string, value: string): void => {
            ...
            <ToolLayout style={{ marginTop: 0 }}>
                ...
                <Panel>
                    ...
                    {activeRows ? (
                        ...
                        <EmptyState>
                            <span style={{ fontSize: 22, fontFamily: "var(--font-mono)" }}>t</span>
