...
export default function UrlValidator() {
    ...
    const handleUrl = (type: string) => {
        switch (type) {
            ...
            case "remove-all":
                {
                    setUrlHistory((prevState: string[]) => [...new Set([...prevState, url])]);
    ...
    const getStatusVariant = (): string => {
        ...
        if (statusNum >= 300) return "secondary";
    ...
    const infoRows = useMemo(() => {
        ...
        try {
            ...
            URL_INFO_KEYS.forEach((key) => {
                const val = urlInfo[key as keyof URL];
                if (val) {
                    rows.push(
                        <InfoRow key={key}>
                            <InfoKey>{key.charAt(0).toUpperCase() + key.slice(1)}</InfoKey>
                            <InfoValue>{String(val)}</InfoValue>
                        </InfoRow>
                    );
                }
            });
