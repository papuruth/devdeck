 ...
 */
export function fuzzyPositions(text: string, query: string): number[] {
...
export function fuzzyFilter<T>(items: T[], query: string, getFields: (item: T) => string[]): T[] {
    ...
    return items
        .map((item: T) => {
            ...
            const score = Math.max(...fields.map((f: string) => fuzzyScore(f || "", query)));
            return { item, score };
        })
 ...
 */
export function fuzzyFilterWithPositions<T>(items: T[], query: string, getFields: (item: T) => string[]): Array<{ item: T; score: number; positions: number[] }> {
    if (!query.trim()) return items.map((item: T) => ({ item, score: 0, positions: [] }));
    ...
    const result = items
        .map((item: T) => {
            const fields = getFields(item);
            const scores = fields.map((f: string) => fuzzyScore(f || "", query));
