...
function renderSectionContent(section: BlogSectionData) {
    if (section.steps) {
        return (
            <BlogSteps>
...
export default function BlogPost() {
    ...
    if (!post) {
        return <NoData title="Blog post not found" />;
    }
    ...
    return (
        <BlogPageWrapper>
            ...
            {post.sections.map((section: BlogSectionData) => (
            ...
            <BlogCTABox>
                ...
                <Button
                    variant="contained"
                    size="large"
                    onClick={() => router.push(post.cta.toolRoute)}
                    sx={{
                        background: "#22cc99",
                        color: "#000",
                        fontWeight: 700,
            ...
            {post.relatedSlugs && post.relatedSlugs.length > 0 && (
                <BlogRelatedWrapper>
                    ...
                    <BlogRelatedGrid>
                        {post.relatedSlugs.map((relSlug: string) => {
                            const rel = blogData[relSlug as keyof typeof blogData];
                            if (!rel) return null;
                            return (
                                <BlogRelatedCard key={relSlug} elevation={0} onClick={() => router.push(`/blog/${relSlug}`)}>
