...
interface BlogPost {
    ...
    title: string;
    intro: string;
    sections?: Array<{ body?: string; list?: string[]; steps?: string[] }>;
}

function getReadingTime(post: BlogPost) {
    const text = [
        post.intro,
        ...(post.sections || []).map((s: { body?: string; list?: string[]; steps?: string[] }) =>
        ...
        <BlogIndexWrapper>
            ...
            {!search && (
                <FeaturedSection>
                    ...
                    <FeaturedGrid>
                        ...
                        {featured.slice(1).map((post: BlogPost) => {
            ...
            {!search && activeCategory === "All" && (
                <>
                    ...
                    <PopularList>
                        {popularPosts.map((post: BlogPost | undefined, i: number) => {
                            if (!post) return null;
                            const cat = CATEGORY_MAP[post.slug];
                            const catStyle = CATEGORY_COLORS[cat] || {};
                            return (
